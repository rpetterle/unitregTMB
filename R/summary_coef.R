...
#' @title Extract and Summarize Model Coefficients
#' 
#' @description 
#' Generic function to extract, format, and compile coefficients and standard errors 
...
summary_coef.unitregTMB <- function(object, ..., component = "mu", digits = 3, print_tex = FALSE) {
  ...
  ## Safety check: Ensure all objects passed are actually unitregTMB models
  ## This prevents the "$ operator is invalid for atomic vectors" error 
  ## if a user misspells an argument name and it gets absorbed into ...
  ...
  results <- lapply(models, function(model) {
    ...
    ## Helper function to clean and prefix names
    ...
    ## Clean 'mu' names and add prefixes for other components
    ...
    ## Combine everything into single vectors
  ...
  ## Store arguments as attributes to be used by the print method
...
## ----------------------------------------------------------------------------
## print: Custom print method for the summary object
## ----------------------------------------------------------------------------
#' @title Print Method for summary_coef.unitregTMB Objects
#' 
#' @description 
#' Prints the compiled coefficient table generated by \code{summary_coef.unitregTMB}. 
#' The output can be displayed as a standard console table or generated as a LaTeX 
#' formatted table for easy inclusion in academic manuscripts.
#' 
#' @param x An object of class \code{summary_coef.unitregTMB}.
#' @param digits Number of decimal places to use when printing. If \code{NULL}, 
#'   it inherits the value specified during the \code{summary_coef} call.
#' @param print_tex A logical indicating whether to print the table in LaTeX format. 
#'   If \code{NULL}, it inherits the value specified during the \code{summary_coef} call.
#' @param ... Further arguments passed to or from other methods.
#' 
#' @return Invisibly returns the original \code{summary_coef.unitregTMB} object.
#' 
#' @export
print.summary_coef.unitregTMB <- function(x, digits = NULL, print_tex = NULL, ...) {
  
  ## Retrieve parameters from attributes if not explicitly provided
  if (is.null(digits)) {
