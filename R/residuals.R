###############################################################################
## unitregTMB: Regression Models for continuous bounded data ------------------
## Diagnostics: Residuals and Auxiliary Functions
...
get_cdf_continuous <- function(family_code, Y, mu, phi, tau) {
  # TMB stores the family code as an integer.
  # We use as.character() to ensure an exact match in the switch statement.
  switch(as.character(family_code),
         
         
         ## Mean: 4 arguments (Y, mu, phi, give_log)
         ...
         ## Mode: 4 arguments (Y, mu, phi, give_log)
         ...
         ## Quantile: 5 Arguments (Y, mu, phi, tau)
...
correct_for_inflation <- function(Y, cdf_cont, p0, p1) {
  ...
  # Ensure p0 and p1 have the same length as Y
  if(length(p0) == 1) p0 <- rep(p0, n)
  if(length(p1) == 1) p1 <- rep(p1, n)
  
  # Handle potential NAs from collinear variables
  p0[is.na(p0)] <- 0
  p1[is.na(p1)] <- 0
  
  # Safety lock: ensure p0 + p1 never exceeds 1
  sum_p <- p0 + p1
  ...
  # Probability of the continuous mass
  p_cont <- 1 - p0 - p1
...
compute_residuals <- function(cdf_val, type) {
  ...
  # Extreme numerical protection for inversions (avoid Inf and -Inf)
  eps <- 1e-7
...
#' Extract Model Residuals for unitregTMB
#' 
#' @description 
...
residuals.unitregTMB <- function(object, type = c("quantile", "cox-snell", "response"), ...) {
  ...
  # 1. EARLY RETURN: Response Residuals
  if (type == "response") {
    mu_hat <- predict(object, type = "response")
    return(Y - mu_hat)
  }
  
  # 2. Preparations for Quantile and Cox-Snell Residuals
  ...
  # Correction for ZOI Inflation (Randomization)
